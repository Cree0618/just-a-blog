...
description: "SGLang HiSparse plus DSpark on a B300: 1,055 seconds to uvicorn, a null spec_info, $2.35 billed. Gate 3 then failed on 147k+1024 > 147456. The crash is the whole result."
tags:
  - inference
  - SGLang
  - DeepSeek
...
Composing the two takes more than turning both flags on: each assumes a batch object, a page geometry, and a cache that the other may have dismantled. This post is a record of that composition on DeepSeek V4 Flash 0731, one NVIDIA B300, SGLang. The server never became healthy, so there is no tokens-per-second number to report.
...
No missing weights, no wrong GPU — the machine was doing real work.
...
Patching the dereference locally would have been a one-line change and a dishonest experiment. I would have been measuring a warmup guard instead of HiSparse. I would also have been measuring a stack that cannot turn radix on. Paying another 17 minutes of B300 time to confirm the same `AttributeError` would have been vanity.

Modal started a second container at 22:33:49 because the guarded deploy had `min_containers=1` — keep-alive behavior, nothing I designed. I stopped the app. The smoke client never wrote a result file. The AIPerf campaign never selected a concurrency. The report template still says `NOT RUN`.
...
The DSpark part buys correctness at a cost in throughput: target verify sees `gamma+1` query rows (width 6). Each row gets a disjoint C4 hot-slot bank so one query cannot evict a selected record another query in the same verify block still needs. Physical `temp_slots` is 512 + 512×6 = 3584. That wastes HBM on purpose. I have not measured a unioned-bank optimization, because I have not measured the path.
...
The fault sat in my harness; the kernels were fine. Official V2 geometry is the other way around: *each* request, including decode, must fit the device pool. Overflow is created by concurrent unique working set, not by one illegal request.
...
The offline tests now refuse the old 147,000-token Gate 3. I have not re-run the paid A/B. CPU preparation is still green; GPU correctness remains unproven. I will not say V2 works.

## Implications

To be clear about what happened here: nothing shipped. Production in this repo is still vLLM. There is no decode rate, no TTFT, no acceptance percentage, no 350K AIPerf table. I never merged the coordinator, never patched `spec_utils.py` on the paid image, never proved swap-in on a B300.
