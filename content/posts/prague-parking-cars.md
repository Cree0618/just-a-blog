...
2. **Coverage is sparse in time and space.** A car can spend minutes on a side street and cross one camera for a single 30-second snapshot. Association asks whether “this crop at camera A at t0” and “that crop at camera B at t1” might be the same object if a car could drive there in t1 − t0.
3. **A naive tracker teleports.** Treat every high-scoring crop as a hit and a white compact at I. P. Pavlova becomes the same track as a white compact at Barrandov one second later. Use the worker’s loop `dt` — time since the previous *observation event* rather than since *this track* was last seen — and you get the opposite bug: a feasible five-minute hop is gated out because the last camera processed was one second ago.

`ppc/tracking/fleet.py` still has a `FleetConstraint` that tries to maintain `fleet_size=12`; the live worker runs without that module.
...
If CLIP is unavailable, the embedder hashes the crop bytes into a 512-D vector and keeps going. The pipeline survives; the re-identification does not.
...
`PPC_TRACK_MAX` defaults to 12, capping track births without promising any particular fleet size. Unmatched detections only birth a track if `final_score >= 0.80`. Two hits inside 120 seconds confirm the track. Tentative tracks die quickly. Confirmed tracks can still die if existence probability collapses.
...
The `dt_s` here is `_dt_track_s`, time since that track’s last assigned hit — feeding it the worker-loop delta instead would bring back the over-gating bug described above. A regression test allows a 60-second hop after a 120-second gap even if the previous observation in the queue was one second ago. Another test forces a coast when travel time is 1,000 seconds and elapsed time is 1.
...
`track_assignments` stores the detection id or NULL for a coast, plus cost, detector score, appearance similarity, existence probability, hits, misses, and whether the track is confirmed. If a track looks stupid on the map, the row lets you argue with the tracker instead of your memory.
...
- CLIP mining against the five refined references produced a 500-row `candidates_by_ref.csv` and a 500-image review folder. The top row has cosine 0.762 versus `maxresdefault.jpg` at camera 500234 — a retrieval ranking, far from precision at 1, since I have not labeled the 500.
...
- The map is a Leaflet page in `ppc/api/server.py`. “Random Track” seeds an `adhoc:` session from a random stored crop — how I test association when no Captur is in view. A Chinese-postman planner can cover a drawn polygon; it lives in its own module, separate from vehicle tracking.
...
- I do not claim the live system maintains twelve tracks. A local prototype DB I inspected had 2 ad-hoc tracks, 40 dead, and 0 active — one snapshot of one database, with no score attached.
...
**Forcing a fleet of twelve.** `IntegratedTracker` plus `FleetConstraint` will create tracks to fill `fleet_size`. Grep the live worker for `mht.py`, `road_tracker.py`, or `integrated_tracker.py` and you will come up empty. All three have tests; none of them is imported.
...
**Hash embeddings as ReID.** They keep the schema happy, and a hash will never pick out a rooftop light bar.

## Implications

Rare-event tracking is a search problem. The failure mode worth fearing is the confident false track that lives for twenty minutes and teaches you nothing — worse than any missed Captur.

Physics is a better first prior than appearance. CLIP is useful for mining a review folder from five news photos; left to itself it would happily jump a car across the river in one second. The hard gate is one `if`. The important part is *which* `dt` you pass it.
...
The next useful work is labeling the 500-image review set, filling `camera_roi.json`, computing the target-free false-track rate, and only then deciding whether a road-graph model has anything to eat — another tracker module can wait. The prototype is a loop I can run, inspect, and disagree with. Whether the twelve parking cars ever show up on its map stays an open question.
